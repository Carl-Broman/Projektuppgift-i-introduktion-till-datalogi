...
export async function chatMessage(weather, location){
    ...
    const completion = await openai.createChatCompletion({
        model: "gpt-3.5-turbo",
        messages: [{role: "user", content: `Answer as if you are aware of the weather in ${location} as it is ${weather.main.temp-272.15} celsius and ${weather.weather.description}. Write the information as if the user does not know and that you knew it before and recommend things to do there`}],
      });
